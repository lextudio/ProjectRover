...
//2026=02-19: Added macOS support
...
public static class SingleInstance
{
	...
	private static string[] GetApplicationCommandLineArgs()
	{
	...
	public static bool Attach(bool noAutoExit)
	{
		lock (_syncRoot)
		{
			...
			try
			{
				...
				{ //we need to contact previous instance
					var contentObject = new SingleInstanceArguments() {
						...
						CommandLineArgs = GetApplicationCommandLineArgs(),
					};
			...
			{
				Trace.TraceWarning(ex.Message + "  {ProjectRover.AppEnv.SingleInstance}");
			}
	...
	private static string PipeName {
		get {
			lock (_syncRoot)
			{
				if (_pipeName == null)
				{
					var pipeName = MutexName;
					if (!RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
					{
						// On Unix, .NET NamedPipe uses a Unix domain socket under:
						//   <temp>/CoreFxPipe_<pipeName>
						// macOS has a strict path length limit (104 chars), so keep the
						// effective pipe name short and deterministic.
						const int maxUnixSocketPathLength = 104;
						const string unixPipePrefix = "CoreFxPipe_";

						var tempPath = Path.GetTempPath();
						var hasTrailingSeparator =
							tempPath.EndsWith(Path.DirectorySeparatorChar) ||
							tempPath.EndsWith(Path.AltDirectorySeparatorChar);
						var separatorLength = hasTrailingSeparator ? 0 : 1;
						var maxPipeNameLength = maxUnixSocketPathLength - tempPath.Length - separatorLength - unixPipePrefix.Length;
						maxPipeNameLength = Math.Max(1, maxPipeNameLength);
						...
						if (pipeName.Length > maxPipeNameLength)
						{
							...
							if (pipeName.Length > maxPipeNameLength)
							{
								pipeName = pipeName.Substring(0, maxPipeNameLength);
							}
						}
					}
